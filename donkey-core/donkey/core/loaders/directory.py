...
from donkey.core.loaders import BaseFileLoader, BaseLoader


def _get_default_file_loaders() -> dict[str, Type[BaseFileLoader]]:
    try:
        from donkey.loaders.file import DocxLoader, HtmlLoader, PdfLoader
    except ImportError:
        raise ImportError(
            "donkey-loaders-file package not found, please install it with `pip install donkey-loaders-file`",
        )
...
class DirectoryLoader(BaseLoader):
    ...
    Example:
        ```python
        from donkey.core.loaders import DirectoryLoader
