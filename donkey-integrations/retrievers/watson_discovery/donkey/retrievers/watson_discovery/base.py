...
from donkey.core.retrievers import BaseRetriever
...
class WatsonDiscoveryRetriever(BaseRetriever):
    ...
    Example:
        ```python
        from donkey.retrievers.watson_discovery import WatsonDiscoveryRetriever

        doc_retriever = WatsonDiscoveryRetriever(
            url="your_url", api_key="your_api_key", project_id="your_project_id"
        )

        docs = doc_retriever.query_documents("What's Donkey Framework?")
        ```
    """
